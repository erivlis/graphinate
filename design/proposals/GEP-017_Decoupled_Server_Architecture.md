...
The `graphinate.renderers.graphql.server` function currently constructs the ASGI application and immediately runs it using `uvicorn`. This coupling prevents users from embedding the application in other contexts or testing it easily. This proposal suggests separating the app creation from the server execution.
...
The `server` function signature will remain the same, ensuring backward compatibility. The new `create_app` function will be an additive change.
