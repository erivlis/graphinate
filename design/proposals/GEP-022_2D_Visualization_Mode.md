...
The current viewer uses `3d-force-graph` to render the graph in a 3D space. While visually impressive ("Wonder"), 3D visualizations often suffer from occlusion and navigation difficulties, making them less effective for analytical tasks ("Clarity"). This proposal advocates for adding a 2D visualization mode using the `force-graph` library.
...
Integrate `force-graph` (the 2D Canvas/SVG sibling of `3d-force-graph`). It shares a nearly identical API, making integration straightforward.
